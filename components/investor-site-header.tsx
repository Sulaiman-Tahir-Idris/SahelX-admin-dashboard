...
export function InvestorSiteHeader({ user }: { user?: InvestorUser }) {
  ...
  const getTitle = () => {
    ...
    if (pathname.includes("/investor/portfolio")) return "My Portfolio"
    return "Investor Portal"
  }

  const roleLabel = "Investor"
  const initials  = (user?.displayName || "I").charAt(0).toUpperCase()
      ...
      <div className="flex items-center gap-1.5 md:gap-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <Avatar className="h-7 w-7 border-2 border-emerald-500/20">
          ...
          <DropdownMenuContent align="end" className="w-64 rounded-xl border-border shadow-card-lg p-1.5">
            <div className="flex items-center gap-3 px-3 py-3 mb-1">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <span className="mt-1 inline-flex self-start text-[10px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-full">
                  {roleLabel}
                </span>
              </div>
            </div>
