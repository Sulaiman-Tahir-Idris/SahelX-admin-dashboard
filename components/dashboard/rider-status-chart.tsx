...
export function RiderStatusChart() {
  ...
  useEffect(() => {
    const fetchRiderStatus = async () => {
      setIsLoading(true)
      try {
        const [riders, deliveries] = await Promise.all([getRiders(), getDeliveries()])
    ...
    <Card>
      ...
      <CardContent className="h-[300px]">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center h-full text-muted-foreground">No rider data available.</div>
        ) : (
